...
//     return angle360(angle);
   ...
   public static Point getMidpointCoordinate(int x1, int y1, int x2, int y2) {
     Point midpoint = new Point();
     midpoint.set((x1 + x2) / 2, (y1 + y2) / 2);
     return midpoint;
   }

   public static MyPoint getMoreLeftPointCoordinate(int x1, int y1, int x2, int y2, int percent, boolean left) {
     MyPoint point = new MyPoint(0, 0, "");
     if (left) {
       point.x = x1 - (x2 - x1) * percent / 100;
       point.y = y1 - (y2 - y1) * percent / 100;
     } else {
       point.x = x2 + (x2 - x1) * percent / 100;
       point.y = y2 + (y2 - y1) * percent / 100;
     }
     return point;
   }
 }
